...
#
# The card carries the date it was built, because a number on screen has to say
# how fresh it is.
set -euo pipefail
cd "$(dirname "$0")"
USER=${1:-bnuuyCode}
...
IN=$(curl -sS -A "$UA" "https://fonts.googleapis.com/css2?family=Inter:wght@400&text=ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789%20.%25%2D" -o "$tmp/c.css" \
...
awk -v IN="$IN" -v DATA="$DATA" '
{ b[NR]=$1; n[NR]=toupper($2); t+=$1 }
END{
  rows=NR; h=64+rows*46
  printf "<svg xmlns=\"http://www.w3.org/2000/svg\" viewBox=\"0 0 620 %d\" width=\"620\" height=\"%d\" role=\"img\" aria-label=\"languages by bytes committed, updated %s\">\n", h, h, DATA
  ...
  printf "<text x=\"622\" y=\"22\" text-anchor=\"end\" font-family=\"'\''IN'\'',system-ui,sans-serif\" font-size=\"11\" letter-spacing=\"2\" fill=\"#5E5E5E\">UPDATED %s</text>\n", DATA
  for(i=1;i<=rows;i++){
...
printf 'languages.svg: %s bytes (coletado em %s)\n' "$(wc -c < languages.svg)" "$DATA"
